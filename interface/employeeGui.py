...
class EmployeeGui(tk.Frame):
    DB_FILE = 'basedados/employee_database'
    LOG_FILE = 'logs/employeegui.log'
    ...
    def __init__(self, master, controller):
        ...
        self.entries = {}
        self.db = None
        img_emplyee = tk.PhotoImage(file='imagens/employee_25x25.png')
        self.label = ttk.Label(self, text='Empregados', font=('Verdana', 12, 'bold'), image=img_emplyee, compound='left')
        self.label.image = img_emplyee
    ...
    def update_record(self):
        try:
            ...
            if key in self.db.keys():
                ...
                record = Employee(nome='?', idade='?', profissao='?', morada='?', telefone='?', email='?',
...
class EmployeeGui(tk.Frame):
    ...
    def make_widgets(self):
        ...
        img_sair = tk.PhotoImage(file='imagens/quit_16x16.png')
        button_8 = ttk.Button(row3_buttons, text='Sair', command=self.quit, width=25, style='W.TButton', image=img_sair, compound='left')
        button_8.image = img_sair
        button_8.pack(side=tk.RIGHT, padx=5,pady=2)
