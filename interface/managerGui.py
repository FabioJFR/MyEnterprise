...
class ManagerGui(tk.Frame):
    def __init__(self, master, controller, shelve_name='basedados/manager_database'):
        ...
        self.entries = {}
        self.db = None
        self.label = tk.Label(self, text='Chefia', font=('Verdana', 14, 'bold'))
    ...
    def update_record(self):
        ...
        try:
            ...
            if key in self.db.keys():
                ...
                record = Manager(name='?', age='?', job='?', adress='?', phone='?', email='?',social_network='?', country='?', nacionality='?', doc_ident='?', nif='?', pay='?', section='?', start_date='?', end_date='?', height='?', weight='?', resume='?')
